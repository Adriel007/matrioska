# Matrioska - LLM Orchestration System with Shared State

![Matrioska](https://live.staticflickr.com/8646/16075618524_6f3b5b199e_b.jpg)

## 📋 Overview

**Matrioska** is an advanced orchestration system for large language models (LLMs) that implements a modular architecture with shared state. Inspired by the concept of Russian nesting dolls, the system decomposes complex tasks into specialized modules that communicate through a shared whiteboard.
*Google Colab link: [Here](https://colab.research.google.com/drive/14LHFEkI6isWJWtGZwnjeUl66SVf6YoT3)*

## 🎯 Key Features

* **🧩 Architectural Decomposition**: Automatically divides complex tasks into specialized modules
* **🧠 Shared State**: Enables inter-module communication via `shared_state`
* **💾 Context Persistence**: Saves and restores progress between executions
* **⚡ Hyperfocus Execution**: Each module operates with a specific focus
* **🔗 Intelligent Integration**: Combines artifacts while maintaining consistency
* **⏳ Simplicity and Reuse**: Aims to produce lightweight code and leverage CDNs/libraries

## 🏗️ Architecture

### Main Components

1. **`LocalLLM`** – Wrapper for Mistral models with 4-bit quantization
2. **`MatrioskaOrchestrator`** – Main orchestrator of the pipeline
3. **`ContextManager`** – Handles state management and persistence
4. **`Architecture`** – Data structure for modular planning

### Execution Flow

```
PHASE 1: ARCHITECTURE → PHASE 2: EXECUTION → PHASE 3: ASSEMBLY
    ↓                         ↓                      ↓
 Decomposition         Modular Execution      Final Integration
```

## 🚀 How to Use

### Installation

```bash
pip install transformers accelerate bitsandbytes torch sentencepiece protobuf
```

### Basic Execution

```python
from matrioska import LocalLLM, MatrioskaOrchestrator

# Initialize model
llm = LocalLLM("mistralai/Mistral-7B-Instruct-v0.3")
orchestrator = MatrioskaOrchestrator(llm, base_path="/content")

# Run task
result = orchestrator.run("Create a library management system with dashboard")
```

### Directory Structure

```
/content/
├── matrioska_artifacts/     # Artifacts generated by each module
├── matrioska_checkpoints/   # Shared state and architecture
│   ├── shared_state.json    # Shared whiteboard
│   └── architecture.json    # Architectural plan
└── matrioska_results.zip    # Downloadable results
```

## 📖 Module System

### Module Specification

```python
@dataclass
class ModuleSpec:
    id: str                    # Unique identifier
    name: str                  # Descriptive name
    description: str           # Functional description
    inputs: str                # Input dependencies
    outputs: str               # Expected outputs
    dependencies: List[str]    # Predecessor modules
    rules: str                 # Specific rules
    shared_state_reads: List[str]  # Keys to read
    shared_state_writes: List[str] # Keys to write
```

### Communication Example

```python
# Module A generates IDs
shared_state_updates = {
    "element_ids": ["#loginForm", "#bookList", "#dashboardStats"],
    "page_structure": {"login": "...", "catalog": "..."}
}

# Module B consumes IDs
context = context_manager.get_shared_context(["element_ids"])
```

## 🔧 Model Configuration

### 4-bit Quantization

```python
quant_config = BitsAndBytesConfig(
    load_in_4bit=True,
    bnb_4bit_compute_dtype=torch.float16,
    bnb_4bit_use_double_quant=True,
    bnb_4bit_quant_type="nf4"
)
```

### Generation Parameters

* **max_tokens**: 4000
* **temperature**: 0.3
* **top_p**: 0.85
* **do_sample**: True

## 📊 Architecture Prompt

The system uses a specialized prompt (`ARCHITECT_SYSTEM_PROMPT`) to decompose tasks, defining:

* **General project objective**
* **Specialized modules** with dependencies
* **Communication contracts** via shared_state
* **Specific manuals** for each module

## 💡 Use Cases

### Web Development

```python
result = orchestrator.run('''
Create a React application with:
- JWT authentication
- Product CRUD
- Admin dashboard
- Responsive design
''')
```

### Data Processing

```python
result = orchestrator.run('''
Data analysis system with:
- REST API extraction
- Cleaning and transformation
- Interactive visualizations
- Automated reports
''')
```

## 🎨 Example Output

```
🪆 MATRIOSKA ORCHESTRATOR - Hyperfocus + SharedState
================================================================================

🏗️  PHASE 1: ARCHITECTURE
--------------------------------------------------------------------------------
📋 Task: 'Library management system with dashboard'

✓ Project: Library Management System
✓ Goal: Create a complete library management system with authentication, CRUD, and dashboard
✓ Modules: 3
   1. HTML Structure 📖[] ✍️['element_ids', 'page_structure']
   2. CSS Styling 📖['element_ids', 'page_structure'] ✍️['css_classes', 'color_scheme']
   3. Authentication Logic 📖['element_ids'] ✍️['auth_api', 'storage_keys']

⚡ PHASE 2: EXECUTION (Hyperfocus + Communication)
--------------------------------------------------------------------------------
🎯 HTML Structure
   ✓ Generated (1542 chars)

🎯 CSS Styling
   📖 Reading context: ['element_ids', 'page_structure']
   ✍️ Wrote: ['css_classes', 'color_scheme']
   ✓ Generated (2387 chars)

🔧 PHASE 3: ASSEMBLY
--------------------------------------------------------------------------------
🔗 Integrating artifacts...

✅ FINAL RESULT
================================================================================
📦 Library Management System
🎯 Create a complete library management system with authentication, CRUD, and dashboard

📂 Artifacts: 3
🧠 SharedState Keys: ['element_ids', 'page_structure', 'css_classes', 'color_scheme', 'auth_api']

🔗 Integrated Result:
--------------------------------------------------------------------------------
[Complete integrated system...]
```

## 🔄 State Management

### Shared State

* **Persistent**: Survives restarts
* **Structured**: JSON-serializable dictionary
* **Selective**: Modules access only relevant keys

### Checkpoints

* Architecture saved in `architecture.json`
* Shared state in `shared_state.json`
* Individual artifacts in text files

## 📦 Exporting Results

```python
# Full results download
from google.colab import files
!zip -r matrioska_results.zip /content/matrioska_artifacts /content/matrioska_checkpoints
files.download('matrioska_results.zip')
```

## 🛠️ Technical Requirements

* **GPU**: NVIDIA T4 (16GB VRAM) or higher
* **RAM**: 16GB+
* **Python**: 3.8+
* **Libraries**: transformers, torch, bitsandbytes, sentencepiece

## 🔮 Roadmap

* [ ] Multi-LLM support
* [ ] Web interface for monitoring
* [ ] Plugin system for custom modules
* [ ] Memory optimization for large projects
* [ ] Integration with version control

## 📄 License

This project is intended for research and development purposes.

---

**Matrioska**: Transforming complexity into intelligent modularity 🪆